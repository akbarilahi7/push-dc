...
(async () => {
    try {
        ...
        console.log("////////////////////////// ")
        console.log("\n")
        console.log(`Silahkan pilih\n1. Generate token\n2. BOT Discord (Autochat + autodelete)\n3. BOT Discord (Autochat only)\n4. Manual BOT Discord (Autochat + autodelete)\n5. Manual BOT Discord (Autochat only)\n6. Exit\n`)
        ...
        if (choise === "1") {
            ...
            console.log("\n[-] Silahkan Input Email & password Discord")
            ...
            const delaychat = readlineSync.question('[!] Masukan Delay (millisecond) : ');
            ...
            console.log(`\n// Badut Discord nodejs (autochat + autodelete)`)
            console.log("// Input Token Manual")
            const token = readlineSync.question('[!] Masukan Token : ');
            const channelId = readlineSync.question('[!] Masukan Channel ID/file.txt : ');
            const delaychat = readlineSync.question('[!] Masukan Delay (millisecond) : ');
            if (channelId.includes(".txt")) {
                if (!(await fsa.exists(channelId))) return console.warn(`Maaf!! File ${channelId} tidak ada`)
                const chnlID = await fsa.readTextFile(channelId, "utf-8")
